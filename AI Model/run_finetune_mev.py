...
def main(_):
    ...
    def is_ponzi(address):
        if address in ponzi_account_set:
            return 1.0
        else:
            return 0.0
    ...
    for addr, pred_proba_list in address_to_pred_proba.items():
        ...
        agg_label_list.append(is_ponzi(vocab.id_to_tokens[addr]))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer("epoch", 2, "Epoch for finetune")
    flags.DEFINE_string("train_input_file", "./inter_data/finetune_mev_train.tfrecord", "Input train file for finetuning")
    flags.DEFINE_string("test_input_file", "./inter_data/finetune_mev_test.tfrecord", "Input test file for finetuning")

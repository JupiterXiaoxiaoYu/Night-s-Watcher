using TCB7_FrontRunAddLiq as collateralToken;
...
rule PoolLiquidityRecordsIsSameOrLessThanTokenContractLiquidityRecords(method f) filtered {f -> !f.isView && f.selector!=sig:claimFee().selector} {
    env e;
    calldataarg args;
    require getCollectoralToken() == collateralToken && getLendingToken() == lendingToken && collateralToken.getSwapAddress(e) == currentContract;
    ...
    swapPercentage = collateralToken.getSwapPercentage(e);

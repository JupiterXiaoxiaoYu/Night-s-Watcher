using TCB3_Inflation_Airdrop as collateralToken;
...
rule PoolLiquidityRecordsIsSameOrLessThanTokenContractLiquidityRecords(method f) filtered {f -> !f.isView && f.selector!=sig:claimFee().selector} {
    ...
    airdropAmount = collateralToken.getAirDropAmount(e);
    outAmount = collateralToken.getOutAmount(e);
    totalSupplyA = collateralToken.totalSupply(e);

    require collateralBalance + outAmount + airdropAmount + airdropAmount < totalSupplyA;
...
rule PoolLiquidityIsAlwaysAvailable{
    ...
    require userCollateralBalance + collateralBalance + outAmount + airdropAmount + airdropAmount< totalSupplyCollateral;
